...
public:
    ...
    static float Noise(const Vector2& coord);
    static float Noise(float x, float y, float z);
    static float Noise(const Vector3& coord);

    static float Fbm(float x, int octave);
    static float Fbm(const Vector2& coord, int octave);
    static float Fbm(float x, float y, int octave);
    static float Fbm(const Vector3& coord, int octave);
...
float Perlin::Noise(float x)
{
    ...
    x -= floor(x);
    float u = Fade(x);
    return Lerp(u, Grad(perm[X], x), Grad(perm[X + 1], x - 1)) * 2.0f;
}
...
float Perlin::Noise(const Vector2& coord)
{
...
float Perlin::Noise(const Vector3& coord)
{
    return Noise(coord.x, coord.y, coord.z);
}

float Perlin::Fbm(float x, int octave)
{
    float f = 0.0f;
    float w = 0.5f;
...
float Perlin::Fbm(const Vector2& coord, int octave)
{
    Vector2 c = coord;
    float f = 0.0f;
    float w = 0.5f;
...
float Perlin::Fbm(float x, float y, int octave)
{
    return Fbm(Vector2(x, y), octave);
}

float Perlin::Fbm(const Vector3& coord, int octave)
{
    Vector3 c = coord;
    float f = 0.0f;
    float w = 0.5f;
...
float Perlin::Fbm(float x, float y, float z, int octave)
{
    return Fbm(Vector3(x, y, z), octave);
}
